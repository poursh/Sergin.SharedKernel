...
- `Sergin.SharedKernel.Application` — command/query abstractions, pipeline behaviors, security, localization, time, and the `ISerginSender`/`IDispatchRouteResolver` dispatch contracts (presentation-agnostic — every Sergin front end, API or UI, dispatches through `ISerginSender` rather than resolving MediatR's `ISender` directly).
- `Sergin.SharedKernel.Infrastructure` — `SerginDbContext` base, `IDbConnectionFactory` implementation, EF interceptors, `DefaultDateTimeProvider`, and `RoutingSerginSender`/`ModuleDispatchRouteResolver` (the `ISerginSender` implementation — fresh scope per call, permission check, then routes `Local`/`Remote` per `Sergin:Dispatch:Modules`).
...
- `Sergin.SharedKernel.Presentation.Blazor` — the shared Blazor kit: `SerginSenderExtensions.SendListAsync` (a thin UI-only helper for list queries, which have no dedicated command type), MudBlazor-backed error presentation, the module nav/route catalog, and the shared shell layout/nav-menu components every module's UI reuses. The dispatcher itself, `ISerginSender`/`RoutingSerginSender`, is presentation-agnostic and lives in `Sergin.SharedKernel.Application`/`Sergin.SharedKernel.Infrastructure` (see below), not here.
