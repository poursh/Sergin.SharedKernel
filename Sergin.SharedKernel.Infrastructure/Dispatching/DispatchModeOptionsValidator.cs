...
/// </summary>
/// <remarks>
/// Its only construction site is <c>AddSerginCore</c>, in <c>Sergin.SharedKernel.Hosts</c>. Registration of
/// <see cref="DispatchModeOptions"/>, <c>ModuleDispatchRouteResolver</c>, and
/// <c>ISerginSender</c>/<c>RoutingSerginSender</c> all live in <c>AddSerginCore</c> itself now, so every
/// Sergin host — Web API included, not just the Blazor UI host — gets dispatch for free and must configure
/// <c>Sergin:Dispatch:Modules</c> at startup, whether or not it ever calls <c>ISerginSender</c>.
/// </remarks>
